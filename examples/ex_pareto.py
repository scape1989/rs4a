...
def pdf_pareto(x, sigma=1., k=1.):
    return np.prod(sp.stats.pareto.pdf(np.abs(x), b=k, scale=1., loc=-1))

def pdf_gaussian(x, sigma=1., k=2.):
    return np.exp(-(np.linalg.norm(x, ord=2) / sigma))
...
if __name__ == "__main__":
    ...
    for i, j in itertools.product(range(50), range(50)):
        z_grid[i, j] = pdf_pareto([x[i], y[j]], args.sigma, args.k)
