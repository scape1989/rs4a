...
def sample_l2_sphere(device, shape):
    noise = torch.randn(shape, device=device).reshape(shape[0], -1)
    noise = noise / torch.norm(noise, dim=1, p=2, keepdim=True) 
    return noise
...
class Exp2Noise(Noise):
    r'''Noise of the form \|x\|_2^{-j} e^{-\|x/\lambda\|_2^k}
    '''
    ...
    def _sigma(self):
        ...
        j = self.j
        dim = self.dim
        return np.sqrt(math.exp(math.lgamma((dim + 2 - j) / k) \
                                - math.lgamma((dim - j) / k)) / dim)
...
class Power2Noise(Noise):
    ...
    def __init__(self, device, dim, sigma=None, lambd=None, a=None, k=None):
        self.a = a
        self.k = k
        if a is None or k is None:
            raise ValueError('Parameter `a` or `k` is required.')
        super().__init__(device, dim, sigma, lambd)
        self.beta_dist = sp.stats.betaprime(dim / k, a - dim / k)

    def __str__(self):
        return f"Power2,a={self.a},k={self.k}"

    def _sigma(self):
        d = self.dim
        k = self.k
        a = self.a
        return np.sqrt(math.exp(math.lgamma(a - (2 + d) / k) + \
                                math.lgamma((2 + d) / k) - \
                                math.lgamma(a - d / k) - \
                                math.lgamma(d / k)) / d)

    def sample(self, x):
        samples = self.beta_dist.rvs((len(x), 1)) ** (1 / self.k)
...
class PowerInfNoise(Noise):
    ...
    def __init__(self, device, dim, sigma=None, lambd=None, a=None):
        ...
        super().__init__(device, dim, sigma, lambd)
        self.beta_dist = sp.stats.betaprime(dim, a - dim)
...
class Exp1Noise(Noise):
    ...
    def __init__(self, device, dim, sigma=None, lambd=None, k=None):
        self.k = k
        if k is None:
